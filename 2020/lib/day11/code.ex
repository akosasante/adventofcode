...
defmodule Advent2020.Day11 do
  ...
  defp run_generation(waiting_room, part) do
    {_, new_waiting_room} =
      Enum.reduce(waiting_room, {0, []}, fn row, {curr_row, new_waiting_room} ->
        new_row =
          Enum.with_index(row)
          |> Enum.map(fn {position, curr_column} ->
            check_neighbours_and_return_new_cell(
              position,
              curr_row,
              curr_column,
              waiting_room,
  ...
  defp check_for_occupied_seats(
    ...
    Stream.unfold({sum, row + check_row, col + check_col, false}, fn
      {sum_of_occupied_seats, row_to_check, col_to_check, false}
      when row_to_check in 0..max_row and col_to_check in 0..max_col ->
        #          :timer.sleep(1500)
        ...
        case cell_at_position(waiting_room, row_to_check, col_to_check) do
          "#" ->
            ...
            {sum_of_occupied_seats + 1,
             {sum_of_occupied_seats, row_to_check + check_row, col_to_check + check_col, true}}
          ...
          _ ->
            ...
            {sum_of_occupied_seats,
             {sum_of_occupied_seats, row_to_check + check_row, col_to_check + check_col, false}}
        end
