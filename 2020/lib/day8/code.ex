  defmodule Advent2020.Day8 do
    ...
    defp run_program(instructions_list, accumulator_value, index_to_run, visited_set) do
      if MapSet.member?(visited_set, index_to_run) do
        ...
        case instruction do
          "nop" -> run_program(instructions_list, accumulator_value, index_to_run + 1, visited_set)
          "acc" -> run_program(instructions_list, accumulator_value + value, index_to_run + 1, visited_set)
          "jmp" -> run_program(instructions_list, accumulator_value, index_to_run + value, visited_set)
        end
      end
    end

    defp part_2_reducer(instructions_list) do
      jmp_or_nop = instructions_list |> Enum.filter(fn {{instruction, _}, _} -> instruction in ["jmp", "nop"] end)
    ...
    defp run_program_p2(instructions_list, accumulator_value, index_to_run, index_to_swap, visited_set) do
      cond do
        ...
        index_to_run >= length(instructions_list) ->
          log("Reached the end of the instructions: #{inspect({index_to_run, length(instructions_list)})}")
          {:end, accumulator_value}
        true ->
          log("Running next instruction")
          visited_set = MapSet.put(visited_set, index_to_run)
          {{instruction, value}, _} = get_instruction_at_index(instructions_list, index_to_run, index_to_swap)
          case instruction do
            "nop" -> run_program_p2(instructions_list, accumulator_value, index_to_run + 1, index_to_swap, visited_set)
            "acc" -> run_program_p2(instructions_list, accumulator_value + value, index_to_run + 1, index_to_swap, visited_set)
            "jmp" -> run_program_p2(instructions_list, accumulator_value, index_to_run + value, index_to_swap, visited_set)
          end
      end
    end

    defp get_instruction_at_index(instructions_list, index_to_run, index_to_swap) when index_to_run == index_to_swap do
    ...
    defp get_instruction_at_index(instructions_list, index_to_run, _), do: Enum.at(instructions_list, index_to_run)
  end
