defmodule Advent2020.Day10 do
  ...
  # figured out by hand/with graph lib; only used as many as the example inputs needed
  @edges_to_combinations_map %{1 => 1, 2 => 1, 3 => 2, 4 => 4, 5 => 7}
  ...
  defp build_edges(starting_graph, all_adapters) do
    Enum.reduce(all_adapters, starting_graph, fn adapter, graph ->
      connectable_adapters =
        Enum.filter(all_adapters, fn other_adapter -> (other_adapter - adapter) in 1..3 end)

      log(
        "Adapter #{adapter} can connect to: #{inspect(connectable_adapters, charlists: :as_lists)}"
      )
  ...
  defp get_paths_to_end(graph, adapter \\ 0, possibilities \\ 1) do
    case Graph.out_degree(graph, adapter) do
      ...
      n ->
        ...
        log(
          "possibilities for adapter #{adapter} [#{n}-degree]: #{possibilities} * #{
            Map.get(@edges_to_combinations_map, n)
          }"
        )
  ...
  defp chunk_contiguous_parts(sorted_adapters) do
    Enum.reduce(sorted_adapters, {[], [], nil}, fn adapter,
                                                   {all_chunks, current_chunk, last_adapter} ->
      ...
      log(
        "curr iter: #{
          inspect({adapter, all_chunks, current_chunk, last_adapter}, charlists: :as_lists)
        }"
      )
